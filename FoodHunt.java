...
public class FoodHunt {
    ...
    static class Product {
        ...
        boolean matchesAllTags(String[] tags) {
            for (String tag : tags)
                if (!Pattern.compile(Pattern.quote(tag), Pattern.CASE_INSENSITIVE).matcher(this.name).find())
                    return false;
    ...
    Vector<Product> findInCategory(String category, Vector<Product> collection){
        Vector<Product> result = new Vector<>();
